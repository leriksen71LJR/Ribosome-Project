...
- At the end of the build, save a file named `BUILD-YYYY-MM-dd-Agent.md`
- The report must be organized into logical grouped sections.
- Include a **Documentation Audit** section using the criteria in `.docs/Evaluation/EvaluationCriteria.md`

### 2. Explicit Deviation & Violation Reporting

Agents **must** explicitly document any deviations or violations.

**Output Location Violation (Critical)**
All generated files must be written directly into the assigned project root folder. Writing to worktrees or temporary harness folders is a serious process violation.

**How to report:**
- Use a section titled **"Deviations and Violations"**
- For each item, explain the rule violated, why, and the impact.
...
- The report **must** be organized into logical grouped sections rather than one long flat list. This makes review easier and forces better synthesis.
- The report should include at minimum:
  - **Summary of Understanding** (grouped by topic — e.g. Architecture, Action Handling, Dependency Injection, Coding Standards)
  - **Key Implementation Decisions**
  - **Deviations from Documentation** (see section below)
...
**Example of good deviation reporting:**

> **Deviations from Documentation**
> - Used synchronous `Execute()` instead of `ExecuteAsync()` on all handlers.
>   - Reason: Time pressure during initial build; async was deprioritized to get a working skeleton.
>   - This is a clear violation of Phase 1.1 rules and will be corrected in the next iteration.
