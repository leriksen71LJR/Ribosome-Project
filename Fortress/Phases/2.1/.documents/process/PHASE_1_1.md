...
**Status:** Mandatory for Phase 1.2A builds using this doc tree.
...
One file in `006/Builds/` per `process/BUILD-DISCLOSURE.md`.
...
## 4. Documentation Layout (Experiment 06)

Agents using this package read **only** `006/` (not legacy flat `.docs/`). Follow `README.md` read order.
...
Standalone `REASONING-*.md` pre-build files are **not** required for standard Phase 1.2A builds. Rule 12 governs: one post-build `BUILD-REPORT-…` only (`process/AGENTS.md` Rule 11 — Post-Build Build Disclosure).