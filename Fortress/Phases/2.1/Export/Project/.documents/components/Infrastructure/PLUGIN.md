...
**Depends on:** Data
**Tension refs:** [0004](../../tensions/0004-storage-save-strategy.md), [0003](../../tensions/0003-archive-semantics-mvp.md)
...
Both are MVP-acceptable per `../../tensions/0004-storage-save-strategy.md`. Do not leave ownership implicit — name the approach in **Key Implementation Decisions → Storage**.
...
**Policy:** Full-table rewrite on `SaveAsync` — delete all rows per table, re-insert from `context.Items` (`../../tensions/0004-storage-save-strategy.md`).
...
`ArchiveCompletedHandler` removes completed tasks from `context.Items` and calls `SaveAsync`. Persisted effect: **hard delete** of completed task rows (`../../tensions/0003-archive-semantics-mvp.md`).
