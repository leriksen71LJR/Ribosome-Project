...
**Tension refs:** [0002](../../tensions/0002-exit-handler-vs-session-guard.md), [0003](../../tensions/0003-archive-semantics-mvp.md)
...
- `ExitHandler` may omit session guard on execute when exiting — see `../../tensions/0002-exit-handler-vs-session-guard.md`.
...
**Status:** Authoritative — Phase 2.1 MVP
...
**ExitHandler exception:** `ExitHandler` is always visible and may run when session is locked — see `../../tensions/0002-exit-handler-vs-session-guard.md`.
...
| `ArchiveCompletedHandler` | Archive Completed Tasks | Remove completed tasks from active storage (hard delete per `../../tensions/0003-archive-semantics-mvp.md`) | `context.Session is not null` **and** `context.Items.OfType<TaskItem>().Any(t => t.IsCompleted)` |
...
**Last Updated:** 2026-06-26 (Phase 2.1 — archive row per tension 0003)
