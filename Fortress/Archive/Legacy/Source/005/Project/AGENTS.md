...
| **Output** | `REASONING-YYYY-MM-DD-XXX-{Agent}.md` in the **project root** (same directory as `AGENTS.md`) |
| **Naming** | `XXX` = 3-digit daily sequence; `{Agent}` = agent slug (no spaces); must match paired `BUILD-REPORT-YYYY-MM-DD-XXX.md` — see `PHASE_1_1_IMPROVEMENTS.md` |
...
| **Handoff** | Return the reasoning file to project stewards for documentation analysis — do not skip or substitute a chat summary |
