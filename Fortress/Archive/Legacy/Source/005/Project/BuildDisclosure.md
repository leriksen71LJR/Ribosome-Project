...
**Output:** `REASONING-YYYY-MM-DD-XXX-{Agent}.md` in the project root (returned to project stewards for analysis)  
**Naming:** `XXX` = 3-digit daily sequence; `{Agent}` = your agent slug (no spaces, e.g. `GrokBuild`, `Claude`); must match paired `BUILD-REPORT-YYYY-MM-DD-XXX.md` (see `PHASE_1_1_IMPROVEMENTS.md` → Report Naming)
...
1. **Last output of the build** — Create `REASONING-YYYY-MM-DD-XXX-{Agent}.md` in the project root. Include **your agent name** in the filename and in the file header. Use the **same** `YYYY-MM-DD-XXX` as the build report for this run. If multiple runs occur the same day, increment `XXX` (`001`, `002`, …) — do not overwrite prior sequences unless instructed.
...
4. **Do not edit `.docs/`** — Suggest improvements in your reasoning file only (except `.docs/Builds/` where the build report already lives).
5. **Stop when done** — Once the reasoning file is saved, the build is complete.

---

## Required Sections in `REASONING-YYYY-MM-DD-XXX-{Agent}.md`

Start the file with this **mandatory header block** (before any section headings):
...
| **Date** | YYYY-MM-DD |
| **Sequence** | XXX |
| **Paired build report** | `.docs/Builds/BUILD-REPORT-YYYY-MM-DD-XXX.md` |
...
## Self-Assessment (include in your reasoning file)
...
If used, add the optional **Craft Reflection** section at the end of your reasoning file. This does not replace any required section above.

---

## Completion

Write `REASONING-YYYY-MM-DD-XXX-{Agent}.md` to the project root (matching the build report sequence; agent in filename). Return it to project stewards. **Stop.**